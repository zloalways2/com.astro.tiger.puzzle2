...
public class GameManager : MonoBehaviour {
  ...
  private int levelPuzzle;

  [SerializeField] GameObject[] labelPuzzle;
  [SerializeField] Image labelFirstPuzzle;
    [SerializeField] Sprite[] spritesPuzzle;

    [SerializeField] GameObject endBlockPuzzle;
    [SerializeField] Image endLabelImage;
    [SerializeField] Sprite winPuzzleSprite;
    [SerializeField] Sprite losePuzzleSprite;
    [SerializeField] Text timerPuzzleLabel;
    ...
    [SerializeField] AudioSource musicPuzzle;
    [SerializeField] AudioSource soundsPuzzle;
  void Start() {
        if (PlayerPrefs.GetString("musicPuzzle", "1") == "1") musicPuzzle.Play();
        // Create the UI
        started = false;
        end = false;
    levelPuzzle = PlayerPrefs.GetInt("levelPuzzle", 0);
    labelFirstPuzzle.sprite = spritesPuzzle[levelPuzzle];
  ...
  public void nextPuzzle()
    {
        levelPuzzle++;
        levelPuzzle %= spritesPuzzle.Length;
        PlayerPrefs.SetInt("levelPuzzle", levelPuzzle);
        PlayerPrefs.Save();
  ...
  public void clickPuzzle()
    {
        for (int i = 0; i < labelPuzzle.Length; i++) labelPuzzle[i].SetActive(false);
        StartGame(imageTextures[levelPuzzle]);
    }
  ...
  public void StartGame(Texture2D jigsawTexture) {
    ...
    levelSelectPanel.gameObject.SetActive(false);
        started = true;
        timePuzzleAftherStart = timer + 120;
  ...
  private void UpdateBorder() {
    ...
    lineRenderer.SetPosition(0, new Vector3(-halfWidth, halfHeight+1, borderZ));
    lineRenderer.SetPosition(1, new Vector3(halfWidth, halfHeight+1, borderZ));
    lineRenderer.SetPosition(2, new Vector3(halfWidth, -halfHeight+1, borderZ));
    lineRenderer.SetPosition(3, new Vector3(-halfWidth, -halfHeight+1, borderZ));
  ...
  void Update() {
    if(!started)    timer += Time.deltaTime;
        ...
        {
            if (timePuzzleAftherStart - timer > 0)
            {
                timePuzzleAftherStart -= Time.deltaTime;
                //timerPuzzleLabel.text = string.Format("TIME: D2:D2", ((int)(((int)timePuzzleAftherStart - timer) / 60)), ((int)(((int)timePuzzleAftherStart - timer) % 60)));
                timerPuzzleLabel.text = "TIME: " + ((int)(((int)timePuzzleAftherStart - timer) / 60)).ToString("D2") + ":" + ((int)(((int)timePuzzleAftherStart - timer) % 60)).ToString("D2");
                //print(((int)(((int)timePuzzleAftherStart - timer) / 60)).ToString("D2"));
            }
            else if (!end)
            {
                gameHolder.gameObject.SetActive(false);
                end = true;
                endBlockPuzzle.SetActive(true);
                endLabelImage.sprite = losePuzzleSprite;
            }
        }
  ...
  private void SnapAndDisableIfCorrect() {
    ...
    Vector2 targetPosition = new Vector2((-width * dimensions.x / 2) + (width * col) + (width / 2),
                                 (-height * dimensions.y / 2) + (height * row) + (height / 2));
    ...
    if (Vector2.Distance(draggingPiece.localPosition, targetPosition) < (width / 2)) {
      ...
      piecesCorrect++;
            if (PlayerPrefs.GetString("soundsPuzzle", "1") == "1") soundsPuzzle.Play();
